...
`;function he(i){return Object.keys(i.states).filter(t=>t.startsWith("switch.wateringhub_"))}function U(i,t){return typeof i?.attributes.friendly_name=="string"?i.attributes.friendly_name:t}function fe(i){return i.states["sensor.wateringhub_status"]?.state??"idle"}function at(i){let t=i.states["sensor.wateringhub_status"];if(t?.state!=="error")return null;let e=t.attributes,r=e.current_program,n=r?i.states[`switch.wateringhub_${r}`]:void 0;return{programName:U(n,r??""),errorMessage:e.error_message??""}}function lt(i){let t=i.states["sensor.wateringhub_status"];if(t?.state!=="running")return null;let e=t.attributes,r=e.valves_done??0,n=e.valves_total??1,o=Array.isArray(e.valves_sequence)?e.valves_sequence:[],s=Array.isArray(e.active_valves)?e.active_valves:[],d=s.map(v=>{let S=Math.max(0,(Date.now()-new Date(v.valve_start).getTime())/1e3),Q=Math.max(0,v.valve_duration-S),b=o.filter(x=>x.water_supply_id===v.water_supply_id&&x.status==="pending").reduce((x,E)=>x+E.duration,0);return Q+b}),a=d.length>0?Math.max(...d):0,p=[...new Set(o.map(v=>v.water_supply_id))].map(v=>o.filter(S=>S.water_supply_id===v).reduce((S,Q)=>S+Q.duration,0)),u=p.length>0?Math.max(...p):0,g=o.filter(v=>v.status==="done").reduce((v,S)=>v+S.duration,0),y=s.reduce((v,S)=>v+Math.max(0,(Date.now()-new Date(S.valve_start).getTime())/1e3),0),f=g+y;return{programName:e.current_program??"",valvesDone:r,valvesTotal:n,progressPercent:e.progress_percent??0,totalRemaining:a,totalDuration:u,totalElapsed:f,valveSequence:o,activeValves:s,dryRun:e.dry_run===!0}}function Me(i){if(i<=0)return"0:00";let t=Math.floor(i/3600),e=Math.floor(i%3600/60),r=Math.floor(i%60);return t>0?`${t}:${e.toString().padStart(2,"0")}:${r.toString().padStart(2,"0")}`:`${e}:${r.toString().padStart(2,"0")}`}function _e(i,t){return i?i.time:""}function ct(i){return!i||i.state==="None"||i.state==="unknown"||i.state==="unavailable"}function Ie(i,t,e,r){let n=i.states[t];if(ct(n))return e("time.never");let o=new Date(n.state);if(Number.isNaN(o.getTime()))return n.state;let s=new Date,d=s.getTime()-o.getTime();if(d<0){let u=new Date(s.getFullYear(),s.getMonth(),s.getDate()),g=new Date(o.getFullYear(),o.getMonth(),o.getDate()),y=Math.round((g.getTime()-u.getTime())/(1e3*60*60*24)),f=o.toLocaleTimeString(r??[],{hour:"2-digit",minute:"2-digit"});return y===0?e("time.today_at",{time:f}):y===1?e("time.tomorrow_at",{time:f}):e("time.in_days",{count:y})}let a=Math.floor(d/(1e3*60));if(a<1)return e("time.just_now");if(a<60)return e("time.minutes_ago",{count:a});let c=Math.floor(a/60);if(c<24)return e("time.hours_ago",{count:c});let p=Math.floor(c/24);return e("time.days_ago",{count:p})}function dt(i,t,e){let r=i.states["sensor.wateringhub_next_run"];return ct(r)?t("time.no_schedule"):Ie(i,"sensor.wateringhub_next_run",t,e)}function pt(i){let t=i.attributes.skip_until;if(typeof t!="string")return null;let e=new Date(t+"T23:59:59");if(Number.isNaN(e.getTime()))return null;let r=new Date;if(r>e)return null;let n=new Date(r.getFullYear(),r.getMonth(),r.getDate()),o=new Date(e.getFullYear(),e.getMonth(),e.getDate());return{isSkipped:!0,daysRemaining:Math.max(1,Math.ceil((o.getTime()-n.getTime())/(1e3*60*60*24))),skipUntil:t}}function ut(i,t){return t("skip.active",{count:i})}function Y(i,t){return l`<span class="badge-sm badge-${i}">${t}</span>`}function ce(i,t,e="primary"){return l`<button class="btn btn-${e}" @click=${t}>${i}</button>`}function W(i,t,e){return l`
  ...
  `}function D(i,t){return l`<button class="add-btn" @click=${t}>${i}</button>`}function J(i,t,e){return l`
  ...
  `}function jt(i,t,e,r,n,o,s,d){if(!e)return l`
    ...
    `;if(fe(i)==="running"){let g=i.states["sensor.wateringhub_status"]?.attributes.current_program,y=t.entity_id?.replace("switch.wateringhub_","");return l`
      <div class="program-status">
        ${g===y?Y("running",n("status.running")):m}
      </div>
    `}let c=pt(t),p=r?Ut(s,n):m;return l`
    <div class="program-status">
      ${c?l`
            ${Y("skipped",ut(c.daysRemaining,n))}
  ...
  `}function ft(i,t,e,r,n,o,s,d,a,c){return t.length===0?l`<div class="empty-state">${c("no_programs")}</div>`:l`${t.map(p=>{let u=i.states[p];if(!u)return m;let g=u.state==="on",y=e===p,f=U(u,p);return l`
...
`;function z(i){return i.states["sensor.wateringhub_status"]?.attributes.available_valves??[]}function q(i){return i.states["sensor.wateringhub_status"]?.attributes.zones??[]}function X(i){return i.states["sensor.wateringhub_status"]?.attributes.water_supplies??[]}function yt(i,t){return t.filter(e=>e.zone_id===i)}function ye(i){return i.toLowerCase().normalize("NFD").replaceAll(/[\u0300-\u036f]/g,"").replaceAll(/[^a-z0-9]+/g,"_").replaceAll(/^_|_$/g,"")}function Ne(i,t,e,r,n,o,s,d){let a=z(i),c=q(i),p=X(i),u=c.length>0&&p.length>0;if(a.length===0)return l`
          ...
          <div class="list-item-header">
            <div style="flex:1">
              ...
              ${u?l`
                    ...
                    ${g&&(!v.zone_id||!v.water_supply_id)?l`<div style="color:var(--error-color);font-size:11px;margin-top:4px;">${d("config.hint_valves_prereq")}</div>`:m}
                  `:m}
            </div>
            ${W("mdi:delete",()=>s(v.entity_id),{title:d("config.delete"),className:"action-btn delete"})}
          </div>
        </div>
      `)}
  ...
  `}function Jt(i,t,e,r,n,o,s,d,a){let c=r&&!i.zone_id,p=r&&!i.water_supply_id,u="border-color: var(--error-color)";return l`
    <div style="display:flex; gap:8px; margin-top:8px; flex-wrap:wrap;">
      <div style="flex:1; min-width:120px;">
        ...
        >
          <option value="">—</option>
          ${t.map(g=>l`<option value=${g.id}>${g.name}</option>`)}
        </select>
      </div>
      <div style="flex:1; min-width:120px;">
  ...
  `}function Gt(i,t,e,r,n){let s=t.filter(d=>d.zone_id===i.id).map(d=>d.name).join(", ")||"\u2014";return G(i.name,s,()=>e(i),()=>r(i.id),n)}function bt(i,t,e,r,n){return l`
  ...
  `}function Kt(i,t,e,r,n){let o=U(i,t),s=i.attributes.schedule,d=i.attributes.total_duration,a=i.attributes.dry_run===!0,c=_e(s,n),p=d?`${c} \u2014 ${d} min`:c,u=a?l`<span class="badge-dry-run-sm">${n("config.dry_run")}</span>`:m;return G(l`${o} ${u}`,p,()=>e(t),()=>r(t),n)}function xt(i,t,e,r,n,o,s){let d=i.zones.reduce((a,c)=>a+c.valves.reduce((p,u)=>p+(u.duration||0),0),0);return l`
    <div class="inline-form">
      <!-- Name -->
      ...
      <div class="form-row">
        ...
        ${[...i.zones.map(a=>t.find(c=>c.id===a.zone_id)),...t.filter(a=>!i.zones.some(c=>c.zone_id===a.id))].filter(a=>!!a).map(a=>{let c=i.zones.findIndex(f=>f.zone_id===a.id),p=c>=0?i.zones[c]:null,u=!!p,g=f=>{let v=[...i.zones],S=c+f;[v[c],v[S]]=[v[S],v[c]],o({...i,zones:v})},y=u&&i.zones.length>1?l`
  ...
  `}function Xt(i,t,e,r,n,o,s,d){let a=o.find(b=>b.id===i.valve_id)?.name??i.valve_id,c=i.frequency,p=c?.type??"",u=new Date().toISOString().slice(0,10),g=t===0,y=t===e.valves.length-1,f=b=>{let x=e.valves.map(T=>T.valve_id===i.valve_id?{...T,...b}:T),E=n.zones.map(T=>T.zone_id===r.id?{...T,valves:x}:T);s({...n,zones:E})},v=b=>{let x=[...e.valves],E=t+b;[x[t],x[E]]=[x[E],x[t]];let T=n.zones.map(ee=>ee.zone_id===r.id?{...ee,valves:x}:ee);s({...n,zones:T})},S=p==="every_n_days"&&c?l`
          <input
  ...
  `}function Fe(i,t,e,r,n,o,s,d,a){let c=X(i);return l`
    ...
    ${c.map(p=>t?.id===p.id?wt(t,o,s,d,a):G(p.name,p.id,()=>e(p),()=>r(p.id),a))}
